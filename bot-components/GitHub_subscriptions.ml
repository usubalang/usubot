...
let github_action ~event ~action json =
  ...
  | "issues", "opened" -> Ok (IssueOpened (issue_info_of_json json))
...
let github_event ~event json =
  ...
  | _ -> Ok (UnsupportedEvent "Unsupported GitHub event.")
      ...
      if Eqaf.equal signature expected then return true
        ...
        Error
          (Caml.Format.asprintf
             "@[<v 1>Webhook signed but with wrong signature.@,\
              Signature: %s@,\
              Expected: %s@,\
              Secret: %s@,\
              Header: %a@,\
              Body: %s@,\
              ---------------@."
             signature expected secret Header.pp_hum headers body)
  | None -> return false)
  >>= fun signed ->
