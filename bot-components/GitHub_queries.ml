  ...
  >|= Result.bind ~f:(fun result ->
          ...
          | Some result -> (
              ...
              | Some pr -> (
                  ...
                  | _, None ->
                      Error
                        (f "Pull request %s/%s#%d does not have a milestone."
                           owner repo number)
...
let pull_request_reviews_info_of_resp ~owner ~repo ~number resp :
  ...
  | Some repository -> (
      ...
      | Some pull_request -> (
          ...
          with
          | None, _, _, _ -> Error "No base ref found."
          | _, None, _, _ -> Error "No files found."
          | _, _, None, _ -> Error "No reviews found."
          | _, _, _, None -> Error "No opinionated reviews found."
          | Some baseRef, Some files, Some reviews, Some opinionatedReviews -> (
              match (reviews.nodes, opinionatedReviews.nodes) with
              | None, _ -> Error "No reviews found."
              | _, None -> Error "No opinionated reviews found."
...
let closer_info_option_of_closer closer =
  ...
  match closer with
  | None | Some (`FutureAddedValue _) -> Ok ClosedByOther
  ...
  >|= Result.bind ~f:(fun resp ->
          ...
          | Some repository -> (
              ...
              | Some (`Commit commit) ->
                  ...
                  in
                  Ok (check || status)
              | _ -> Error (f "Unkown commit %s/%s@%s." owner repo commit)))
  ...
  >|= Result.bind ~f:(fun repository ->
          ...
          | Some pull_request, Some (`Commit base_obj), Some (`Commit head_obj)
            -> (
              ...
              | Some [ baseCheckSuite ], Some [ headCheckSuite ] -> (
                  ...
                  | Some baseCheckRuns, Some headCheckRuns ->
                      ...
                      let completed_runs checkRuns description commit =
                        ...
                        |> List.map ~f:(fun checkRun ->
                               ...
                               | Some conclusion -> (
                                   ...
                                   | `FAILURE | `NEUTRAL ->
                                       Ok
                                         ( get_check_tab_info checkRun,
                                           Some false )
                                   | `SUCCESS ->
                                       Ok
                                         (get_check_tab_info checkRun, Some true)
                                   | `FutureAddedValue status ->
                                       Error
                                         ( checkRun.name,
                                           f
                                             "Unknown status %s for check run \
                                              %s (for %s commit %s/%s@%s"
                      ...
                      let labels : string list option =
                        let open GetChecks in
                        let open Option in
                        pull_request.labels >>= fun labels ->
                        labels.nodes >>= fun labels ->
              ...
              | _ -> Error "Got more than one checkSuite."))
    ...
    | Ok result -> (
        ...
        | Some result -> (
            ...
            | Some result -> (
                ...
                | Some result ->
                    let next =
                      if result.pageInfo.hasNextPage then
                    ...
                    in
                    Lwt.return (Ok (data, next))
                | None -> Lwt.return @@ Error (f "Error querying labels"))
            | None ->
                Lwt.return
                @@ Error
                     (f "Unknown pull request %s/%s#%d" owner repo pr_number))
